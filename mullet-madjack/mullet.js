...
const JOGO_ID = "json:2";

// botão coração do header
const headerFavoriteBtn = document.querySelector('.icon-btn[title="Favoritos"]');
...
function atualizarBotaoFavorito() {
    ...
    wishlistIcon.className = isFav
        ? 'fas fa-heart-broken'
        : 'far fa-heart';

    wishlistText.textContent = isFav
...
btnWishlist?.addEventListener('click', () => {
    ...
    if (favoritos.includes(JOGO_ID)) {
        ...
        showToast('<i class="far fa-heart"></i> Removido dos favoritos');

    } else {

        favoritos.push(JOGO_ID);

        showToast('<i class="fas fa-heart"></i> Adicionado aos favoritos!');
